...
将本目录复制到 DSH 的 agent-presets 目录，然后重启 DSH 并在新会话中选择该预设：

```bash
cp -r .agent-presets/dsh-preset-router-standard ~/.dsh/.agent-presets/dsh-preset-router-standard
```

> 如果已经位于 `~/.dsh/.agent-presets/` 下，复制到其他机器时保持目录名唯一即可。
